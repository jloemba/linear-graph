...
class RockFamilyIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        rockFamily = new GraphAggregate("La Grande Famille du Rock");
    ...
    private Node genre(String name) {
        return new Node(name, null, GENRE);
    }

    private Node artist(String name) {
        return new Node(name, null, ARTIST);
    }
    ...
    void shouldModelPunkLineage() {
        // Rock'n'Roll → British R&B → Blues Rock → Hard Rock → Punk Era → Punk Rock → Cold Wave
